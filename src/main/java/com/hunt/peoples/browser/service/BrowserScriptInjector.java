...
public class BrowserScriptInjector {
    ...
    // limits (защита от огромных payload)
    @Value("${browser.cdp.inject.max-script-bytes:250000}") // ~250KB
    private int maxScriptBytes;

    @Value("${browser.cdp.inject.max-total-bytes:1500000}") // ~1.5MB
    private long maxTotalBytes;

    @Value("${browser.cdp.inject.log-snippet-len:120}")
    private int logSnippetLen;

    /**
     * ✅ Инжект через уже открытую CDP-сессию (page target).
     * Только Page.addScriptToEvaluateOnNewDocument (без второго WS, без Runtime.evaluate).
     *
     * @return сколько скриптов реально добавили
     */
    public int injectForProfile(Long profileId, DevToolsSession cdp) {
        ...
        List<String> scripts = safeGenerate(profile);
        if (scripts.isEmpty()) {
        ...
        for (String src : scripts) {
            ...
            if (bytes > maxScriptBytes) {
                log.warn("Skip script: too large ({} bytes > {}), profileId={}, snippet={}",
                        bytes, maxScriptBytes, profileId, snippet(src));
                continue;
            }
            ...
            // ✅ важно: addScriptToEvaluateOnNewDocument работает корректно в PAGE target ws
            // (у тебя это уже проверяется: wsUrl содержит /devtools/page/)
            try {
                ...
                // Не валим весь запуск из-за одного скрипта
                log.warn("Failed to inject script (profileId={}, bytes={}, err={}): {}",
                        profileId, bytes, e.getClass().getSimpleName(), e.getMessage());
            }
        }
    ...
    private List<String> safeGenerate(Profile profile) {
        try {
            ...
            log.warn("Failed to generate scripts for profileId={} ({}): {}",
                    profile.getId(), e.getClass().getSimpleName(), e.getMessage());
            return List.of();
        }
    }
    ...
    private String snippet(String s) {
        if (s == null) return "";
        String t = s.replace("\n", " ").replace("\r", " ").trim();
        if (t.length() <= logSnippetLen) return t;
        return t.substring(0, Math.max(0, logSnippetLen)) + "...";
    }
    ...
    public String generateInjectionScript(Profile profile) {
        ...
        sb.append("// profileId=").append(profile.getId()).append("\n");

        for (int i = 0; i < scripts.size(); i++) {
            String s = scripts.get(i);
            if (s == null || s.isBlank()) continue;
            sb.append("\n// ---- module ").append(i + 1).append(" ----\n");
            sb.append(s).append("\n");
        }
        return sb.toString();
    }
...
//        List<String> scripts;
//        try {
//            scripts = qaScriptGenerator.generateAll(profile);
//        } catch (Exception e) {
//            log.warn("Failed to generate scripts for profileId={} ({})",
...
//
//        // Склеиваем модули в один source
//        StringBuilder sb = new StringBuilder();
