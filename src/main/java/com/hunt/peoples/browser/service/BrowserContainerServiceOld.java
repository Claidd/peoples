...
//@Slf4j
//public class BrowserContainerServiceOld {
//
...
//    @PostConstruct
//    public void init() {
//        // ШАГ 1: Инициализация (PostConstruct)
...
//        // ШАГ 2: Запуск браузера (основной процесс)
//        //  Подготовка данных
...
//
//        // Проверка лимитов запущенных контейнеров
//        checkContainerLimit();
//
...
//
//        // Настройка volume (привязка директории)
//        String hostPath = normalizePath(userDataDir);
//        ensureDirectoryExists(hostPath);
//
//        HostConfig hostConfig = HostConfig.newHostConfig()
//                .withPortBindings(portBindings)
//                .withBinds(new Bind(hostPath, new Volume("/data/user-data"))) // Привязывает директорию профиля на хосте к /data/user-data в контейнере
//                .withMemory(containerMemoryMB * 1024L * 1024L) // Устанавливает ограничения памяти (по умолчанию 2GB)
//                .withMemorySwap(containerMemoryMB * 2L * 1024L * 1024L)
//                .withCpuShares(containerCpuShares)
//                .withPrivileged(false)
//                .withShmSize(shmSizeMB * 1024L * 1024L) // Устанавливает размер shared memory (512MB, важно для Chrome)
...
//        }
//
//        // Подготовка переменных окружения env, который передает параметры профиля в контейнер через переменные окружения
//        // Эти переменные читаются скриптом start.sh внутри контейнера
...
//
//        // Формирование URL и ожидание готовности. Формирует URL для доступа к VNC и DevTools
...
//
//        // ШАГ 3: Настройка браузера через CDP
//        // CDP: emulation (viewport/touch/locale/tz/ua)
//        configureBrowserViaCdp(devToolsUrl, profileId);
//
//
//        // ШАГ 4: Обработка версии браузера
//        // Читаем реальную версию Chromium/Chrome (для логов/метаданных). fetchDevtoolsVersionSafe: Запрашивает /json/version у DevTools
...
//        }
//
//        // ШАГ 5: Обновление статуса профиля
...
//
//        // ШАГ 6: Возврат результата. Возвращает клиенту все необходимые данные для доступа. VNC URL для ручного управления
//        // DevTools URL для автоматизации. Срок жизни сессии (1 час)
...
//
//    // Передает параметры профиля в контейнер через переменные окружения
//    // Эти переменные читаются скриптом start.sh внутри контейнера
...
//
//        // ✅ Берём PAGE target из /json/list (НЕ /json/new)
//        //  Получение WebSocket URL. Делает HTTP запрос к http://localhost:9224/json/list
//        // Получает список всех доступных DevTools targets. Находит target типа "page" (основная вкладка)
//        // Извлекает webSocketDebuggerUrl (WebSocket URL для CDP)
...
//
//        // Подключение к CDP. Устанавливает WebSocket соединение с DevTools браузера
//        // Включает основные домены CDP (Page, Network, Runtime)
//        // try-with-resources гарантирует закрытие соединения
//        try (DevToolsSession cdp = devToolsClient.connect(pageWsUrl, websocketConnectTimeout, 30)) {
//
//            // 0) включаем домены 1 раз
//            cdp.enableCommonDomains(3000);
//
//            // ✅ (пункт 1 из “двух пунктов”): включаем Fetch + простой фильтр
//            // Включение перехвата запросов (Fetch) для перехвата сетевых запросов. Позволяет блокировать или модифицировать запросы
//            // В данном примере блокируются картинки ("Image")
//            cdp.safeSend("Fetch.enable", Map.of(
//                    "patterns", List.of(Map.of("urlPattern", "*", "requestStage", "Request"))
//            ), 3000);
//
//            // Подписка на события Fetch.requestPaused
//            AutoCloseable unsubFetch = cdp.onEvent("Fetch.requestPaused", evt -> {
//                try {
//                    JsonNode p = evt.path("params");
//                    String requestId = p.path("requestId").asText("");
//                    String reqUrl = p.path("request").path("url").asText("");
//                    String resourceType = p.path("resourceType").asText("");
//
//                    if (requestId.isBlank()) return;
//
//                    // пример: блочим картинки (можешь расширить правилами)
//                    if ("Image".equals(resourceType)) {
//                        cdp.safeSend("Fetch.failRequest", Map.of(
//                                "requestId", requestId,
...
//
//                    // иначе пропускаем
//                    cdp.safeSend("Fetch.continueRequest", Map.of("requestId", requestId), 2000);
//
//                } catch (Exception e) {
//                    // ВАЖНО: не даём обработчику “убить” WS-поток
//                    log.debug("Fetch.requestPaused handler error: {}", e.getMessage());
//                }
//            });
//
//            // Эмуляция устройства
...
//
//            // Инъекция anti-detect скриптов Скрипты маскируют fingerprint (скрывают WebDriver, меняют навигатор и т.д.)
//            // 2) inject (только addScriptToEvaluateOnNewDocument)
//            int injected = 0;
//            if (injectScripts) {
//                injected = scriptInjector.injectForProfile(profileId, cdp); // ✅ ВАЖНО: присвоить!
//            }
//
//            // Перезагрузка страницы. Перезагружает страницу, чтобы инжектированные скрипты применились. ignoreCache: true - игнорирует кэш браузера
//            // ✅ (пункт 2 из “двух пунктов”): reload, чтобы скрипты точно применились
//            if (injectScripts && injected > 0) {
//                cdp.send("Page.reload", Map.of("ignoreCache", true), 15000);
//            }
//
//            // Верификация настройки. Проверяет, что настройки применились корректно
//            verifyCdpBasic(cdp, profileId);
//
//            // отписка от Fetch событий (чисто, хотя try-with-resources и так закроет WS)
//            try { unsubFetch.close(); } catch (Exception ignore) {}
//
//        } catch (Exception e) {
...
//    }
//
//    // Верификация настройки. Выполняет JavaScript в контексте страницы
//    // Проверяет, что настройки применились корректно. Логирует результат для отладки
...
//    }
//
//    public String getDevToolsWebSocketUrl(String devToolsUrl) {
//        RestTemplate restTemplate = restTemplate(1500, 1500);
//
//        try {
//            String base = devToolsUrl.endsWith("/") ? devToolsUrl.substring(0, devToolsUrl.length() - 1) : devToolsUrl;
//
//            // например http://127.0.0.1:55980/json/list
//            var resp = restTemplate.getForObject(base + "/json/list", JsonNode.class);
//            if (resp == null || !resp.isArray() || resp.isEmpty()) return null;
//
//            // Ищем target типа "page", обычно это "New Tab"
//            for (JsonNode t : resp) {
//                String type = t.path("type").asText("");
//                String ws = t.path("webSocketDebuggerUrl").asText("");
//                if ("page".equals(type) && !ws.isBlank()) return ws;
//            }
//
//            // fallback: иногда first элемент — page
//            JsonNode first = resp.get(0);
//            String ws = first.path("webSocketDebuggerUrl").asText("");
//            return ws.isBlank() ? null : ws;
//
//        } catch (Exception e) {
//            log.warn("Failed to resolve PAGE wsUrl from {}: {}", devToolsUrl, e.getMessage());
//            return null;
//        }
//    }
//
//    public String getDevToolsBaseUrl(Long profileId) {
//        ContainerInfo info = ACTIVE_CONTAINERS.get(profileId);
//        if (info == null) return null;
//
//        String base = appProperties.getHostBaseUrl();
//        base = base.endsWith("/") ? base.substring(0, base.length() - 1) : base;
//
//        return base + ":" + info.getHostDevToolsPort();
//    }
//
//    // Настраивает браузер так, чтобы он выглядел как указанное устройство. Устанавливает точные размеры экрана, DPR (pixel ratio)
//    // Эмулирует touch-интерфейс для мобильных устройств
//    // Переопределяет User Agent, локаль, таймзону
...
//        // Включает/выключает тач-эмуляцию
...
//        }
//
//        // Устанавливает локаль и таймзону
...
//
//
//    // fetchDevtoolsVersionSafe: Запрашивает /json/version у DevTools
...
//
//
//    private String getBrowserWsUrl(String devToolsUrl) {
//        RestTemplate rt = restTemplate(2000, 2000);
//        String base = devToolsUrl.endsWith("/") ? devToolsUrl.substring(0, devToolsUrl.length() - 1) : devToolsUrl;
//
//        try {
//            JsonNode ver = rt.getForObject(base + "/json/version", JsonNode.class);
//            String ws = ver != null ? ver.path("webSocketDebuggerUrl").asText(null) : null;
//            return (ws == null || ws.isBlank()) ? null : ws;
//        } catch (Exception e) {
//            return null;
//        }
//    }
...
//public String openTabAndGetPageWs(String devToolsUrl, String url) {
//    String browserWs = getBrowserWsUrl(devToolsUrl);
//    if (browserWs == null) return null;
//
//    try (DevToolsSession browser = devToolsClient.connect(browserWs, websocketConnectTimeout, 30)) {
//        browser.send("Target.setDiscoverTargets", Map.of("discover", true), 2000);
//
//        JsonNode resp = browser.send("Target.createTarget", Map.of("url", url), 5000);
//        String targetId = resp.path("result").path("targetId").asText(null);
//        if (targetId == null || targetId.isBlank()) return null;
//
//        // теперь достаём page ws из /json/list по id
//        RestTemplate rt = restTemplate(2000, 2000);
//        String base = devToolsUrl.endsWith("/") ? devToolsUrl.substring(0, devToolsUrl.length() - 1) : devToolsUrl;
//
//        JsonNode list = rt.getForObject(base + "/json/list", JsonNode.class);
